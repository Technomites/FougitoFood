...
import { View, Text, TextInput, Image, ImageBackground, StyleSheet , TouchableOpacity, Keyboard, ScrollView,KeyboardAvoidingView} from 'react-native';
...
import {forgotpassword, forgotpasswordreset} from '../Actions/actions';
...
const ForgotPassword = ({navigation}) => {
    ...
    const {Lang, forgotpasswordresponse, forgotpasswordresponsestatus} = useSelector(state => state.userReducer);
    ...
    const countryCodes = [
      ...
      useEffect(() => {
        ...
        if (forgotpasswordresponsestatus == "success") {
            showToast(forgotpasswordresponse, {
                duration: 500,
              });
              setTimeout(async () => {
                // navigation.navigate("OtpVerification")
                navigation.navigate('OtpVerification', {
                    phoneNumber: phoneNumber,   pin: selectNumber, screentitle: "forgotpassword"
                  });
              }, 3000);
        ...
        if (phoneNumber == '') {
          ...
          NetInfo.fetch().then(state => {
            if (state.isConnected == false && state.isInternetReachable == false) {
              ...
              dispatch(forgotpassword(phoneNumber, selectNumber.substring(1)));
            }
          });
        }
    ...
    return (
      ...
      behavior={Platform.OS === 'ios' ? 'padding' : 'position'}>
            ...
            {Lang == "en" ? "Enter the phone number associated with your account. We will send a verification for confirmation" : "أدخل رقم الهاتف المرتبط بحسابك. سوف نرسل التحقق للتأكيد"}
            </Text>
            <View style={{position:'relative',width:"100%",   marginBottom: "2%",}}>
                {/* <TextInput
                    style={[styles.inputStyle, {backgroundColor: phoneNumberActive || phoneNumber !== "" ? "#FFF" : "transparent"}]}
                    value={phoneNumber}
           ...
           ResetPassword()
            ...
            style={{position:'absolute',bottom:scalableheight.two,  alignSelf:"center"}}>
                <View style={{flexDirection: Lang == "en" ?  "row" : "row-reverse"}}>
                    <Text style={{color:"#B3B3B3",fontFamily: FontFamily.regularFont,
     fontSize: fontSize.fourteen,
    }}>
                    {Lang == "en" ? "Login with a different account? " : "تسجيل الدخول بحساب مختلف؟"}
                    </Text>
                    <Text style={{color: theme.color.white,fontFamily: FontFamily.regularFont,
     fontSize: fontSize.fourteen, marginLeft: Lang == "en" ? "2%" : "0%", marginRight: Lang == "en" ? "0%" : "2%"}}>
